...
- **会话**：每个飞书聊天拥有**独立专属的 Agent 会话池**（绝不串进 GUI 会话）；首条消息自动创建会话（绑定配置工作区，无需先在 GUI 打开）；每聊天会话持久化（重启后自动恢复）；创建的会话会自动挂到配置的工作区（不再掉进「未分组」），旧版本遗留的会话在恢复时也会补挂；重启后即使个别恢复失败，也不会清空历史会话列表（/list、/switch 仍可回到旧会话）；`agents.create/resume` 均注入模型选项（修复 `{{model}}` 变量缺失）
